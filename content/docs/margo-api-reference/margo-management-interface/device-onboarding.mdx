...
- The device's management client downloads the Workload Fleet Manager's public root CA certificate using the [Onboarding API](../../margo-api-reference/margo-management-interface/rootca-download.mdx)
- Context and trust is established between the device's management client and the Workload Fleet Management web service
- The device's management client uses the [Onboarding API](../../margo-api-reference/margo-management-interface/client-onboarding.mdx) to onboard with the Workload Fleet Management service.
...
- The [device capabilities](./device-capability-reporting.mdx) information is sent from the device to the WFM service using the [Device API](../../margo-api-reference/margo-management-interface/device-capabilities.mdx)
